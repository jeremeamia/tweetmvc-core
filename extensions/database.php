...
function c(){static$m;return$m=@$m?:new\mysqli(\c(db_host),\c(db_user),\c(db_pass),\c(db_name));}                                           #Connection singleton
...
function v(&$v,$k=0,$p=0){return$v=($p?\sprintf($p,$k):'').(\is_numeric($v)?$v:($v===NULL?'NULL':'"'.\mysql_real_escape_string($v).'"'));}  #Prepare a value
